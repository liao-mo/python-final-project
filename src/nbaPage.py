...
class NBAPage(tk.Frame):
    def __init__(self, master, line_api_key):
        ...
        def print_games():
            selected_teams = [
                team_listbox.get(index) for index in team_listbox.curselection()
            ]
            ...
            if dataSet:
                for team in selected_teams:
                    for data in dataSet.data:
                        home_team_name = (
                            data["homeTeam"]["teamCity"]
                            + " "
                            + data["homeTeam"]["teamName"]
                        )
                        away_team_name = (
                            data["awayTeam"]["teamCity"]
                            + " "
        ...
        choose_teams_button = tk.Button(
            frame, text="Choose Teams", command=choose_teams
        )
        ...
        time_combobox = ttk.Combobox(
            ...
            values=["6:00 PM", "7:00 PM", "8:00 PM", "9:00 PM"],
        )
