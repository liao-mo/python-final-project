...
class NBAPage(tk.Frame):
    def __init__(self, master, line_api_key):
        ...
        self.scrollbar = tk.Scrollbar(
            frame, orient=tk.VERTICAL, command=self.team_listbox.yview
        )
        ...
        self.choose_all_teams_button = tk.Button(
            frame, text="Select All", command=self.select_all_teams
        )
        ...
        self.freeline = tk.Label(
            frame,
            text="------------------------------------------------------------------------------------------------------------------------------------------------------",
        )
        self.freeline.grid(row=4, column=0, columnspan=8, sticky="ew")

        self.service1_label = tk.Label(
            frame, text="> Service 1 : Directly send today's schedule and game status."
        )
        self.service1_label.config(font=("Helvetica", 14))
        self.service1_label.grid(row=5, column=0, columnspan=5, sticky="w")

        self.send_today_schedule_button = tk.Button(
            frame, text="Send", command=self.send_today_schedule
        )
        ...
        self.service2_label = tk.Label(
            frame,
            text="> Service 2 : Periodically send the schedule in the near future.",
        )
        ...
        self.menu_symbols = tk.OptionMenu(
            frame, self.selected_time_symbol, *self.time_symbols
        )
        ...
        self.trigger_button = tk.Button(
            frame, text="Activate", command=self.convert_and_trigger
        )
    ...
    def send_today_schedule(self):
        selected_teams = [
            self.team_listbox.get(index) for index in self.team_listbox.curselection()
        ]
            ...
            for team in selected_teams:
                if team not in redundantTeams:
                    for data in dataSet.data:
                        home_team_name = self.makeTeamFullName(
                            data["homeTeam"]["teamCity"], data["homeTeam"]["teamName"]
                        )
                        away_team_name = self.makeTeamFullName(
                            data["awayTeam"]["teamCity"], data["awayTeam"]["teamName"]
                        )
    ...
    def sendGamesSchedule(self, schedule, dayslimit):
        ...
        for game_date in schedule["leagueSchedule"]["gameDates"]:
            ...
            for game in game_date["games"]:
                ...
                daysbetween = int(
                    ((game_time_utc - current_time_utc).total_seconds()) // (3600 * 24)
                )

                if daysbetween <= dayslimit and daysbetween >= 0:
                    selected_teams = set(
                        self.team_listbox.get(index)
                        for index in self.team_listbox.curselection()
                    ) - set(redundantTeams)
    ...
    def convert_and_trigger(self):
        ...
        try:
            ...
            print(
                "Invalid input. Please enter numeric values for hours, minutes, and seconds."
            )
