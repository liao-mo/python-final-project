...
class TradingPage(tk.Frame):
    def __init__(self, master):
        ...
        self.menu_comparison = tk.OptionMenu(self, self.selected_comp_operators, *self.comp_operators)
        ...
        self.btn_comparison = tk.Button(
            ...
            command=lambda: self.add_new_task(
                "comparison",
                10000,
                {"symbol": self.selected_symbol.get(), "operator": self.selected_comp_operators.get(), "value": self.comparison_number.get()},
                )
        )
        ...
        self.btn_delete_task = tk.Button(self, text="Delete", command=lambda: self.deleteTask(self.getCurrentSeletedTaskIndex()))
    ...
    def add_new_task(self, taskType, new_interval, params):
        ...
        if taskType == "periodic":
            temp_str_list.append(f"{params["symbol"]} notify interval: {new_interval / 1000.0}s")
        elif taskType == "comparison":
            temp_str_list.append(f"{params["symbol"]} notify when the price {params["operator"]} {params["value"]}")
